#!/bin/bash
# =============================================================================
#  CHD_Dataset030_MedDINO_Centering_d4.sh
#  Dataset030_imageCHD_HU — MedDINOv3 3D centering inflation, d_patch=4, 500 ep
#
#  Finest depth resolution rung: d=4 = 2400 tokens (~12.2x pretrained) vs
#  d=8 = 1200 (6.1x) and d=16 = 600 (3.1x). Self-attention is O(N^2), so this is
#  ~4x the d=8 attention memory — START AT batch_size=1. Watch the per-epoch
#  [GPU] line in the training log; if peak_reserved leaves clear headroom under
...
# =============================================================================
#SBATCH --job-name=D030-MedDINO-C4
...
#SBATCH --output=/scratch/users/sastocke/nnunet_CHD/logs/D030-MedDINO-C4_%j.out
#SBATCH --error=/scratch/users/sastocke/nnunet_CHD/logs/D030-MedDINO-C4_%j.err
...
CKPT_DIR="${nnUNet_results}/${DATASET_NAME}/.checkpoints/CHD_Dataset030_MedDINO_Centering_d4"
START_TS=$(date +%s)
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║  CHD_Dataset030_MedDINO_Centering_d4.sh — START                ║"
echo "╠══════════════════════════════════════════════════════════════════╣"
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║  CHD_Dataset030_MedDINO_Centering_d4.sh — COMPLETE             ║"
echo "╠══════════════════════════════════════════════════════════════════╣"
