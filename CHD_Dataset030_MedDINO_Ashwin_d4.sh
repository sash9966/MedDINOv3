#!/bin/bash
# =============================================================================
#  CHD_Dataset030_MedDINO_Ashwin_d4.sh
#  Dataset030_imageCHD_HU — MedDINOv3 3D Ashwin channel-averaging inflation, d=4
#
#  Direct counterpart to CHD_Dataset030_MedDINO_Centering_d4.sh. Identical token
#  budget (d=4 = 2400 tokens, ~12.2x pretrained) and identical training setup; the
#  ONLY difference is the patch_embed inflation strategy:
...
# =============================================================================
#SBATCH --job-name=D030-MedDINO-A4
...
#SBATCH --output=/scratch/users/sastocke/nnunet_CHD/logs/D030-MedDINO-A4_%j.out
#SBATCH --error=/scratch/users/sastocke/nnunet_CHD/logs/D030-MedDINO-A4_%j.err
...
CKPT_DIR="${nnUNet_results}/${DATASET_NAME}/.checkpoints/CHD_Dataset030_MedDINO_Ashwin_d4"
START_TS=$(date +%s)
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║  CHD_Dataset030_MedDINO_Ashwin_d4.sh — START                   ║"
echo "╠══════════════════════════════════════════════════════════════════╣"
...
# ─────────────────────────────────────────────
# Phase 3 — 3D Ashwin d=4 training (fold 0)
# ─────────────────────────────────────────────
echo "================================================================"
echo "Phase 3: 3D Ashwin d=4 training — fold 0  (${MEDDINOV3_NUM_EPOCHS} epochs)"
echo "================================================================"
KEY="p3_3d_ashwin_d4_fold0"
if is_done "${KEY}"; then
    echo "[SKIP] ${KEY}"
else
    echo "--- ${KEY} ---"
...
if is_done "p4_infer_3d_ashwin_d4"; then
    echo "[SKIP] p4_infer_3d_ashwin_d4"
else
    nnUNetv2_predict \
        -i "${IN_DIR}" -o "${PRED_DIR}" \
        ...
        -f 0 \
        -tr ${TRAINER_3D}
    mark_done "p4_infer_3d_ashwin_d4"
fi
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════╗"
echo "║  CHD_Dataset030_MedDINO_Ashwin_d4.sh — COMPLETE                ║"
echo "╠══════════════════════════════════════════════════════════════════╣"
